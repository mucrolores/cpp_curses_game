...
class producer
{
	public:
		...
		void move();
		bool checkDand(int next_X,int next_Y);
		int copy();
		void clear();
...
void producer::move()
{
	...
	for(int i=-1;i<2;i++)
	{
		for(int j=-1;j<2;j++)
		{
			if(i!=0 || j!=0)
			{
				if(checkDand(cor_X+i,cor_Y+j)==0 && screen[cor_X+i][cor_Y+j]==' ')
				{
					die_Pos=0;
				}
			}
	...
	if(life>0)
	{
		...
		if(screen[cor_X-1][cor_Y-1]==' '||screen[cor_X-1][cor_Y]==' '||screen[cor_X-1][cor_Y+1]==' '||screen[cor_X][cor_Y-1]==' '||screen[cor_X][cor_Y+1]==' '||screen[cor_X+1][cor_Y-1]==' '||screen[cor_X+1][cor_Y]==' '||screen[cor_X+1][cor_Y+1]==' ')
		{
			if(die_Pos==0)
			{
				while(screen[new_Cor_X][new_Cor_Y]!=' ' || checkDand(new_Cor_X,new_Cor_Y)==1)
				{
					new_Cor_X = cor_X+rand()%3-1;
					new_Cor_Y = cor_Y+rand()%3-1;
				}
...
bool producer::checkDand(int next_X,int next_Y)
{
	for(int i=-1;i<2;i++)
	{
		for(int j=-1;j<2;j++)
		{
			if(i!=0 || j!=0)
			{
				if(screen[next_X+i][next_Y+j]=='C' && screen[next_X][next_Y]==' ')
				{
					return true;
				}
			}	
...
class consumer
{
	public:
		...
		void place_Cor(int target_Cor);
		void move();
		bool checkPrey(int next_X,int next_Y);
		int eat();
		void clear();
...
void consumer::move()
{
	...
	for(int i=-1;i<2;i++)
	{
		for(int j=-1;j<2;j++)
		{
			if(i!=0 || j!=0)
			{
				if(checkPrey(cor_X+i,cor_Y+j)==1 && screen[cor_X+i][cor_Y+j]==' ')
				{	
					eat_pos=1;
				}
			}
	...
	if(life>0)
	{
		...
		if(screen[cor_X-1][cor_Y-1]==' '||screen[cor_X-1][cor_Y]==' '||screen[cor_X-1][cor_Y+1]==' '||screen[cor_X][cor_Y-1]==' '||screen[cor_X][cor_Y+1]==' '||screen[cor_X+1][cor_Y-1]==' '||screen[cor_X+1][cor_Y]==' '||screen[cor_X+1][cor_Y+1]==' ')
		{
			if(eat_pos==1)
			{
				while(screen[new_Cor_X][new_Cor_Y]!=' '|| checkPrey(new_Cor_X,new_Cor_Y)==0)
				{
					new_Cor_X = cor_X+rand()%3-1;
					new_Cor_Y = cor_Y+rand()%3-1;
				}	
...
bool consumer::checkPrey(int next_X,int next_Y)
{
	for(int i=-1;i<2;i++)
	{
		for(int j=-1;j<2;j++)
		{
			if(i!=0 || j!=0)
			{
				if(screen[next_X+i][next_Y+j]=='p' && screen[next_X][next_Y]==' ')
				{
					return true;
				}
			}
...
int main()
{
	...
	while(true)
	{
		...
		//system("PAUSE");
		showScreen();
		cout << "producerAmount:"<< producerAmount << endl << "consumerAmount:" << consumerAmount <<endl;
		/*
		for(int i=0;i<200;i++)
		{
			cout << "[" << i << "]:";
		...
		*/
		...
		else if(producerAmount==1716)
		{
			system("CLS");
			cout << "game over" << endl;
			cout << "system message:producer conquer the world"<< endl;
			break;
		}
		else if(consumerAmount==1716)
		{
			system("CLS");
			cout << "game over" << endl;
